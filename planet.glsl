...
vec3 getColorForCoord(vec2 fragCoord) {
    ...
    if (t < inf) {
        ...
        return vec3(2.0 / 255.0, 6.0 / 255.0, 18.0 / 255.0);
    }
...
mediump vec4 effect(mediump vec4 color, Image texture, mediump vec2 texture_coords, mediump vec2 pixel_coords) {
    vec2 fragCoord = pixel_coords - iCenter.xy;
